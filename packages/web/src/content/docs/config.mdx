...
The default agent must be a primary agent (not a subagent). This can be a built-in primary agent like `"coding"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, OpenContext falls back to `"coding"` with a warning.
