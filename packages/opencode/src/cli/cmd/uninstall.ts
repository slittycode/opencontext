...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall opencontext and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall OpenContext")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g opencontext",
      pnpm: "pnpm uninstall -g opencontext",
      bun: "bun remove -g opencontext",
      yarn: "yarn global remove opencontext",
      brew: "brew uninstall opencontext",
      choco: "choco uninstall opencontext",
      scoop: "scoop uninstall opencontext",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall opencontext -y -r`.quiet().nothrow()
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".opencontext") || binDir.includes(".opencode")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using OpenContext!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    const content = await Bun.file(file)
      ...
      .catch(() => "")
    if (
      content.includes("# opencontext") ||
      content.includes(".opencontext/bin") ||
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (trimmed === "# opencontext" || trimmed === "# opencode") {
      skip = true
      continue
    }
    ...
    if (skip) {
      skip = false
      if (
        trimmed.includes(".opencontext/bin") ||
    ...
    if (
      (trimmed.startsWith("export PATH=") &&
        (trimmed.includes(".opencontext/bin") || trimmed.includes(".opencode/bin"))) ||
      (trimmed.startsWith("fish_add_path") && (trimmed.includes(".opencontext") || trimmed.includes(".opencode")))
    ) {
      continue
    }
