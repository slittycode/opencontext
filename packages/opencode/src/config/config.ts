...
export namespace Config {
  ...
  function getManagedConfigDir(): string {
    switch (process.platform) {
      case "darwin":
        return "/Library/Application Support/opencontext"
      case "win32":
        return path.join(process.env.ProgramData || "C:\\ProgramData", "opencontext")
      default:
        return "/etc/opencontext"
    }
  }

  const managedConfigDir = process.env.OPENCODE_TEST_MANAGED_CONFIG_DIR || getManagedConfigDir()
  const PROJECT_CONFIG_FILES = ["opencontext.jsonc", "opencontext.json", "opencode.jsonc", "opencode.json"] as const
  const CONTEXT_DIR_NAMES = [".opencontext", ".opencode"] as const
  ...
  export const state = Instance.state(async () => {
    ...
    // 2) Global config (~/.config/opencontext/opencontext.json{,c})
    // 3) Custom config (OPENCODE_CONFIG)
    // 4) Project config (opencontext.json{,c}; legacy opencode.json{,c})
    // 5) .opencontext/.opencode directories with matching config files
    ...
    for (const [name, mode] of Object.entries(result.mode ?? {})) {
      const agentName = name === "build" ? "coding" : name
   ...
   * 2. Local opencontext.json
   * 3. Global plugin/ directory
   * 4. Global opencontext.json
   *
  ...
  export const Server = z
    .object({
      ...
      mdnsDomain: z.string().optional().describe("Custom domain name for mDNS service (default: opencontext.local)"),
  ...
  export const Info = z
    .object({
      ...
      server: Server.optional().describe("Server configuration for opencontext serve and web commands"),
      command: z
      ...
      default_agent: z
        ...
        .optional()
        .describe(
          "Default agent to use when none is specified. Must be a primary agent. Falls back to 'coding' if not set or if the specified agent is invalid.",
        ),
      username: z
        .string()
        .optional()
  ...
  export const global = lazy(async () => {
    let result: Info = pipe(
      ...
      mergeDeep(await loadFile(path.join(Global.Path.config, "opencontext.json"))),
      mergeDeep(await loadFile(path.join(Global.Path.config, "opencontext.jsonc"))),
